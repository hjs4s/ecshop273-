...
class cls_template
{
    ...
    function fetch_str($source)
    {
        ...
        $obj = $this;
        return preg_replace_callback('/{([^\}\{\n]*)}/',function($matche) use ($obj){
            return $obj->select($matche[1]);
        },$source);
    }
...
//            }
        ...
        else
        {
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert' :
                    ...
                    $out = "<?php \n" . '$k = ' . preg_replace_callback("/(\'\\$[^,]+)/" , function($matche){
                            return stripslashes(trim($matche[1],'\''));
                        }, var_export($t, true)) . ";\n";
    ...
    function get_val($val)
    {
        if (strrpos($val, '[') !== false)
        {
            ...
            $val = preg_replace_callback('/\[([^\[\]]*)\]/is',function($matche){
                return '.'.str_replace('$','$',$matche[1]);
            },$val);
        }
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            $source = preg_replace_callback($pattern,function($matche){
                return '{include file='.strtolower($matche['1']). '}';
            },$source);
            /* 检查有无动态库文件，如果有为其赋值 */
